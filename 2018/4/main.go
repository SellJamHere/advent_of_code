...
func main() {
	fmt.Println("Part 1")
	inputOne := parseInput(one)
	sleepyGuard := findSleepyGuard(inputOne)
	commonMinute, _ := findMostCommonMinuteForSingleGuard(sleepyGuard)
	fmt.Printf("guard: %d, minute: %d = %d\n", *sleepyGuard[0].guardID, commonMinute, *sleepyGuard[0].guardID*commonMinute)

	puzzleInput := parseInput(input)
	sleepyGuard = findSleepyGuard(puzzleInput)
	commonMinute, _ = findMostCommonMinuteForSingleGuard(sleepyGuard)
	fmt.Printf("guard: %d, minute: %d = %d\n", *sleepyGuard[0].guardID, commonMinute, *sleepyGuard[0].guardID*commonMinute)

	fmt.Println("Part 2")
	maxGuardID, commonMinute := findMostCommonMinuteForAnyGuard(inputOne)
	fmt.Printf("guardID: %d, minute: %d = %d\n", maxGuardID, commonMinute, maxGuardID*commonMinute)

	maxGuardID, commonMinute = findMostCommonMinuteForAnyGuard(puzzleInput)
	fmt.Printf("guardID: %d, minute: %d = %d\n", maxGuardID, commonMinute, maxGuardID*commonMinute)
}
...
func parseInput(input string) map[int][]shift {
	guardMap := map[int][]shift{} // map guard IDs to shifts

	lines := strings.Split(input, "\n")
	sort.SliceStable(lines, func(i, j int) bool {
		iParts := strings.Split(lines[i], "] ")
		iTime, _ := time.Parse("2006-01-02 15:04", iParts[0][1:len(iParts[0])])

		jParts := strings.Split(lines[j], "] ")
		jTime, _ := time.Parse("2006-01-02 15:04", jParts[0][1:len(jParts[0])])
	...
	for _, line := range lines {
		...
		timeString := parts[0][1:len(parts[0])]
		timestamp, _ := time.Parse("2006-01-02 15:04", timeString)

		actionPart := parts[1]
		if strings.Contains(actionPart, "begins shift") {
			// save last shift
			if nextShift.guardID != nil {
				guardMap[*nextShift.guardID] = append(guardMap[*nextShift.guardID], nextShift)
			}

			guardParts := strings.Split(actionPart, " ")
			guardID, _ := strconv.Atoi(guardParts[1][1:])
			nextShift = shift{
				guardID: &guardID,
				states: []state{
					state{
			...
			nextShift.states = append(nextShift.states, state{
				asleep: true,
				start:  timestamp,
			})
...
func findSleepyGuard(guardMap map[int][]shift) []shift {
	sleepCount := map[int]int{}

	for guardID, shifts := range guardMap {
		for _, shift := range shifts {
			for _, state := range shift.states {
				if state.asleep {
					sleepCount[guardID]++
				}
			}
	...
	var sleepyGuardID int
	var sleepyCount int
	for guardID, count := range sleepCount {
		if count > sleepyCount {
			sleepyGuardID = guardID
			sleepyCount = count
		}
	}

	return guardMap[sleepyGuardID]
}

func findMostCommonMinuteForSingleGuard(guard []shift) (int, int) {
	minuteCount := map[int]int{}

	wasAsleep := false
	var sleepStart time.Time
	for _, shift := range guard {
		for _, state := range shift.states {
			if state.asleep {
				sleepStart = state.start
				wasAsleep = true
				if state.start.Hour() == 0 {
					minuteCount[state.start.Minute()]++
				}
			} else if wasAsleep {
				for i := sleepStart.Minute() + 1; i < state.start.Minute(); i++ {
					if state.start.Hour() == 0 {
						minuteCount[i]++
					}
				}
	...
	return commonMinute, minCount
}

func findMostCommonMinuteForAnyGuard(guardMap map[int][]shift) (int, int) {
	maxGuardID := 0
	commonMinute := 0
	commonMinuteCount := 0

	for guardID, shifts := range guardMap {
		mostCommonMinute, minuteCount := findMostCommonMinuteForSingleGuard(shifts)

		if minuteCount > commonMinuteCount {
			maxGuardID = guardID
			commonMinute = mostCommonMinute
			commonMinuteCount = minuteCount
		}
	}

	return maxGuardID, commonMinute
}

var one = `[1518-11-01 00:00] Guard #10 begins shift
...
var input = `[1518-03-30 00:57] wakes up
