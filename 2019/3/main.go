...
func main() {
	...
	// Part 1
	wireLocations := map[point][]wirelocation{}
	// map out wire locations
	for wireIdx, wire := range wires {
		...
		for _, path := range paths {
			...
			for i := 0; i < distance; i++ {
				...
				wireLocations[wireLocation.point] = append(wireLocations[wireLocation.point], wireLocation)
			}
		}
	}
	...
	for point, locations := range wireLocations {
		if len(locations) <= 1 {
			continue
		}

		intersectionWires := map[int]struct{}{}
		for _, location := range locations {
			intersectionWires[location.wire] = struct{}{}
		}

		if len(intersectionWires) < 2 {
			continue
		}

		intersectionDistance := int(math.Abs(float64(point.x)) + math.Abs(float64(point.y)))
	...
	// Find closest intersection
	closestIntersection = math.MaxInt64
	for _, locations := range wireLocations {
		if len(locations) <= 1 {
			continue
		}
		...
		wire1 := wirelocation{}
		wire2 := wirelocation{}
		closest1 := math.MaxInt64
		closest2 := math.MaxInt64
		for _, location := range locations {
			if location.wire == 0 && location.distance < closest1 {
				wire1 = location
			} else if location.wire == 1 && location.distance < closest2 {
				wire2 = location
			}
		}

		if wire1 == (wirelocation{}) || wire2 == (wirelocation{}) {
			continue
		}

		intersectionDistance := wire1.distance + wire2.distance
		if intersectionDistance < closestIntersection {
			closestIntersection = intersectionDistance
		}
	}

	fmt.Printf("intersection path distance: %d\n", closestIntersection)
}
