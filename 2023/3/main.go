...
var interestingRegex = regexp.MustCompile("[^0-9.]")
...
func main() {
	...
	for j, line := range lines {
		for i, val := range line {
			if interestingRegex.MatchString(string(val)) {

				// Check around the point, starting from the top left
				numSet := map[int]struct{}{}
				for x := i - 1; x <= i+1; x++ {
					for y := j - 1; y <= j+1; y++ {
						if x >= 0 && x < len(line) && y >= 0 && y < len(lines) {

							// If this point is a number, go left to find the start,
							// then right to find the end, then add to a map.
							if numRegex.MatchString(string(lines[y][x])) {
								startX := x
								for numX := x; numX >= 0; numX-- {
									if !numRegex.MatchString(string(lines[y][numX])) {
										break
									}
									startX = numX
								}
								...
								numStr := ""
								for numX := startX; numX <= endX; numX++ {
									numStr += string(lines[y][numX])
								}
				...
				for partNum := range numSet {
					partSum += partNum
				}

				if string(lines[j][i]) == "*" && len(numSet) == 2 {
					var gearValues []int
					for val := range numSet {
						gearValues = append(gearValues, val)
					}
					gearRatioSum += gearValues[0] * gearValues[1]
				}
			}
		}
	}

	fmt.Println("part 1 sum:", partSum)
	fmt.Println("part 2 sum:", gearRatioSum)
}
...
const puzzleInput = `....573.613.........965............691......892..948.......964........439.375..................320......273...........352.284...............
.......*.............*.....814...............$....*........../..94......*....=.............103............/..882*...........+...............
