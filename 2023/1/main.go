...
func main() {
	...
	lines := strings.Split(puzzle1Input, "\n")

	total := 0
	for _, line := range lines {
		firstNum := findFirstDigit(line)
		secondNum := findFirstDigit(reverse(line))
	...
	total = 0
	for _, line := range lines {
		firstDigit := findFirstDigit(convertNumToDigitWithDirection(line, true))

		secondDigit := findFirstDigit(reverse(convertNumToDigitWithDirection(line, false)))
...
func convertNumToDigit(s string, numbers map[string]int) string {
	// Iterate over initial string
	for sIdx := range s {
		// Iterate over possible numbers
		for num, digit := range numbers {
			// Iterate over the length of the number. If it matches, replace and return
			match := true
			currSIdx := sIdx
			for _, digitChar := range num {
				if currSIdx < len(s) && string(digitChar) != string(s[currSIdx]) {
					match = false
					break
				}
				currSIdx++
			...
			if match == true {
				s = strings.Replace(s, num, strconv.Itoa(digit), 1)
				return s
			}
		}
	}
...
func convertNumToDigitWithDirection(s string, forward bool) string {
	...
	if !forward {
		s = reverse(s)
		reversedNumbers := map[string]int{}
		for num, digit := range numbers {
			reversedNumbers[reverse(num)] = digit
		}

		numbers = reversedNumbers
	}

	converted := convertNumToDigit(s, numbers)
...
const puzzle1Input = `tsgbzmgbonethreedrqzbhxjkvcnm3
