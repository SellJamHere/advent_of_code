...
func main() {
	...
	for _, game := range games {
		...
		for _, round := range game {
			mostRed = int(math.Max(float64(round["red"]), float64(mostRed)))
			mostGreen = int(math.Max(float64(round["green"]), float64(mostGreen)))
			mostBlue = int(math.Max(float64(round["blue"]), float64(mostBlue)))
		}

		powerTotal += mostRed * mostGreen * mostBlue
	}

	fmt.Println("part 2 total:", powerTotal)
}
...
func parseInput(s string) [][]map[string]int {
	...
	for i, line := range strings.Split(s, "\n") {
		...
		game, _ := strconv.Atoi(gameParts[1])
		if game != i+1 {
			panic(fmt.Sprintf("unexpected game: %d", game))
		}
		...
		for _, set := range sets {
			...
			setParts := strings.Split(strings.TrimSpace(set), ",")
			for _, countStr := range setParts {
				countParts := strings.Split(strings.TrimSpace(countStr), " ")
				count, _ := strconv.Atoi(countParts[0])
				setMap[countParts[1]] = count
			}

			setList = append(setList, setMap)
		}

		games = append(games, setList)
	}
...
const puzzleInput = `Game 1: 8 green; 5 green, 6 blue, 1 red; 2 green, 1 blue, 4 red; 10 green, 1 red, 2 blue; 2 blue, 3 red
